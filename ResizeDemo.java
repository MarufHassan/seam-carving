...
public class ResizeDemo {
    public static void main(String[] args) {
        if (args.length != 3) {
            StdOut.println("Usage:\njava ResizeDemo [image filename] [new width] [new height]");
            return;
        }

        Picture inputImg = new Picture(args[0]);
        int removeColumns = inputImg.width() - Integer.parseInt(args[1]);
        int removeRows = inputImg.height() - Integer.parseInt(args[2]);
